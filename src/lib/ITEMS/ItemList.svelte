...
<div class="list element--border--primary">
    {#if $user.id === $current_market.creator_id}
        {#if creating_new_item}
            <button on:click={toggleNewItem} class="newItem item element--border--primary">Cancel New Item</button>
        {:else}
            <button on:click={toggleNewItem} class="newItem item element--border--primary">New Item</button>
        {/if}
    {/if}
